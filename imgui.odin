...
build_font_atlas :: proc() -> mem.Allocator_Error {
	...
	font_texture: ^rl.Texture2D = cast(^rl.Texture)io.Fonts.TexID
...
imgui_new_frame :: proc() {
	...
	io.DisplaySize.x = f32(settings.window_width) //f32(rl.GetMonitorWidth(rl.GetCurrentMonitor()))
	io.DisplaySize.y = f32(settings.window_width) //f32(rl.GetMonitorHeight(rl.GetCurrentMonitor()))
...
imgui_process_events :: proc() -> bool {
	io: ^imgui.IO = imgui.GetIO()
